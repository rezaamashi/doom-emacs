...
** Fix straight update on native-comp per Emacs 29
There was problem with ~native-comp-deferred-compilation-deny-list~ being deprecated for emacs 29. Instead ~native-comp-jit-compilation-deny-list~ is should be the one used. The old ~straight.el~  that was being pinned by doom is yet to implement that update.
#+begin_src emacs-lisp :tangle packages.el
(unpin! straight)
(package! straight
  :type 'core
